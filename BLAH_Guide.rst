...
-  ``userFQAN``: user's certificate Fully Qualified Attribute Names
   (i.e. VO and roles) repeated, one per FQAN

-  ``ceID``: Computing Element's ID as appearing in the Glue schema

-  ``jobID``: the Grid job ID (i.e. the one returned to the user by the
   CE)

-  ``lrmsID``: the ID of the job in the batch system

-  ``localUser``: numeric uid of the local account mapped to the remote
   user

-  ``clientId``: a job identifier (arbirtary string) provided by the CE
   at submission time

Accounting can be disabled in BLAH by commenting out the
BLAHPD\_ACCOUNTING\_INFO\_LOG line in blah.config.
